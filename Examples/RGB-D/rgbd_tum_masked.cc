...
void LoadImages(const string &strAssociationFilename, vector<string> &vstrImageFilenamesRGB,
                vector<string> &vstrImageFilenamesD,std::vector<std::string> &vstrMaskFilenames, vector<double> &vTimestamps);
...
int main(int argc, char **argv)
{
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        if(imageScale != 1.f)
        {
            ...
            cv::resize(msk, msk, cv::Size(width, height));
            std::cout<<"Image scale resized!"<<endl;
        }
        ...
        SLAM.TrackRGBD(msk,imRGB,imD,tframe);
